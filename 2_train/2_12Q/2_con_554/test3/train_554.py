...
def Train_Pyt(bfe,img):
  ...
  for i in range (n_batch): # n_batch
    ...
    img1=img[num[beg_frm:end_frm]].reshape(batch,128,32).permute(2,1,0).reshape(32,128*batch) # 789 10 11 01 2 34 5 6
    pred=torch.matmul(Con_Unitary(Par1[0],S_Ker1),img1).reshape(128,32,batch).permute(1,0,2).reshape(32,128*batch) # 23456 789 10 11 01
    pred=torch.matmul(Con_Unitary(Par1[1],S_Ker1),pred).reshape(2,16,2,16,4,batch).permute(4,0,2,1,3,5).reshape(16,256*batch) # 0127
...
def Pred_Pyt(img,par1,par2,par3):
  ...
  for i in range (n_batch):
    ...
    if end_frm>sam:
      ...
      batch=end_frm-beg_frm
    img1=img[num[beg_frm:end_frm]].reshape(batch,128,32).permute(2,1,0).reshape(32,128*batch) # 789 10 11 01 2 34 5 6
    pred=torch.matmul(cm0,img1).reshape(128,32,batch).permute(1,0,2).reshape(32,128*batch) # 23456 789 10 11 01
    pred=torch.matmul(cm1,pred).reshape(2,16,2,16,4,batch).permute(4,0,2,1,3,5).reshape(16,256*batch) # 0127
    pred=torch.matmul(cm2,pred).reshape(2,2048,batch).permute(2,0,1)
    pred=pred.pow(2).sum(dim=2)
...
def quantum_circuit(state,cm0,cm1,cm2):
  ...
  qml.QubitUnitary(cm0,wires=[7,8,9,10,11])
  qml.QubitUnitary(cm1,wires=[2,3,4,5,6])
  qml.QubitUnitary(cm2,wires=[0,1,2,7])
