...
def Train_Pyt(bfe,img):
  ...
  for i in range (n_batch): # n_batch
    ...
    img1=img[num[beg_frm:end_frm]].reshape(batch,2,16,16).permute(1,3,2,0).reshape(32,16*batch) # 05678 1234
    pred=torch.matmul(Con_Unitary(Par1[0],S_Ker1),img1).reshape(2,16,16,batch).permute(0,2,1,3).reshape(32,16*batch) # 01234 5678
    pred=torch.matmul(Con_Unitary(Par1[1],S_Ker1),pred).reshape(2,256,batch).permute(2,0,1)
...
def Pred_Pyt(img,par1,par2):
  ...
  for i in range (n_batch):
    ...
    if end_frm>sam:
      ...
      batch=end_frm-beg_frm
    img1=img[num[beg_frm:end_frm]].reshape(batch,2,16,16).permute(1,3,2,0).reshape(32,16*batch) # 05678 1234
    pred=torch.matmul(cm0,img1).reshape(2,16,16,batch).permute(0,2,1,3).reshape(32,16*batch)
    pred=torch.matmul(cm1,pred).reshape(2,256,batch).permute(2,0,1)
    pred=pred.pow(2).sum(dim=2)
