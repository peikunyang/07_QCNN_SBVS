...
def Train_Pyt(bfe,img):
  ...
  for i in range (n_batch): # n_batch
    ...
    img1=img[num[beg_frm:end_frm]].reshape(batch,2,4,16,4).permute(2,4,1,3,0).reshape(16,32*batch) # 1278 03456
    pred=torch.matmul(Con_Unitary(Par1[0],S_Ker1),img1).reshape(2,8,2,4,4,batch).permute(2,0,4,1,3,5).reshape(16,32*batch) # 01 56 278 34
    pred=torch.matmul(Con_Unitary(Par1[1],S_Ker1),pred).reshape(2,64,4,batch).permute(0,2,1,3).reshape(8,64*batch) # 034
    pred=torch.matmul(Con_Unitary(Par2[0],S_Ker2),pred).reshape(8,64,batch).permute(2,0,1).reshape(batch,2,256)
...
def Pred_Pyt(img,par1,par2,par3):
  ...
  for i in range (n_batch):
    ...
    if end_frm>sam:
      ...
      batch=end_frm-beg_frm
    img1=img[num[beg_frm:end_frm]].reshape(batch,2,4,16,4).permute(2,4,1,3,0).reshape(16,32*batch) # 1278 03456
    pred=torch.matmul(cm0,img1).reshape(2,8,2,4,4,batch).permute(2,0,4,1,3,5).reshape(16,32*batch) # 01 56 278 34
    pred=torch.matmul(cm1,pred).reshape(2,64,4,batch).permute(0,2,1,3).reshape(8,64*batch) # 034
    pred=torch.matmul(cm2,pred).reshape(8,64,batch).permute(2,0,1).reshape(batch,2,256)
    pred=pred.pow(2).sum(dim=2)
