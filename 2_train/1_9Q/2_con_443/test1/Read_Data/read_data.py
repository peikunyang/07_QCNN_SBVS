...
def Read_Img(pdb):
  ...
  for i in range (len(pdb)):
    fr=open("../../../../1_database/1_structure/5_img/1_9_qubits/PDBbind_v2020/%s.img"%(pdb[i]),"r")
    for line in fr:
      lx=line.split()
...
def OutPara(par1,par2,par3):
  ...
  for i in range (N_Lay2):
    par=Con_Unitary(par2[i],S_Ker2).numpy().reshape(-1).tolist()
    for k in range (len(par)):
      fw.write("%12.8f "%(par[k]))
      if k%10==9:
        fw.write("\n")
