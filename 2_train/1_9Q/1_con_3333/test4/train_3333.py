...
def Train_Pyt(bfe,img):
  ...
  for i in range (n_batch): # n_batch
    ...
    img1=img[num[beg_frm:end_frm]].reshape(batch,2,4,4,16).permute(1,3,2,4,0).reshape(8,64*batch) # 034125678
    pred=torch.matmul(Con_Unitary(Par1[0],S_Ker1),img1).reshape(2,4,2,2,4,4,batch).permute(2,4,0,3,1,5,6).reshape(8,64*batch) # 1 56 023478
    pred=torch.matmul(Con_Unitary(Par1[1],S_Ker1),pred).reshape(2,4,2,2,4,4,batch).permute(3,5,2,0,4,1,6).reshape(8,64*batch) # 2 78 013456
    pred=torch.matmul(Con_Unitary(Par1[2],S_Ker1),pred).reshape(2,4,4,16,batch).permute(2,0,3,1,4).reshape(8,64*batch) # 012 345678
    pred=torch.matmul(Con_Unitary(Par1[3],S_Ker1),pred).reshape(2,256,batch).permute(2,0,1)
...
def Pred_Pyt(img,par1,par2):
  ...
  for i in range (n_batch):
    ...
    if end_frm>sam:
      ...
      batch=end_frm-beg_frm
    img1=img[num[beg_frm:end_frm]].reshape(batch,2,4,4,16).permute(1,3,2,4,0).reshape(8,64*batch) # 034125678
    pred=torch.matmul(cm0,img1).reshape(2,4,2,2,4,4,batch).permute(2,4,0,3,1,5,6).reshape(8,64*batch) # 1 56 023478
    pred=torch.matmul(cm1,pred).reshape(2,4,2,2,4,4,batch).permute(3,5,2,0,4,1,6).reshape(8,64*batch) # 2 78 013456
    pred=torch.matmul(cm2,pred).reshape(2,4,4,16,batch).permute(2,0,3,1,4).reshape(8,64*batch) # 012 345678
    pred=torch.matmul(cm3,pred).reshape(2,256,batch).permute(2,0,1)
    pred=pred.pow(2).sum(dim=2)
...
def quantum_circuit(state,cm0,cm1,cm2,cm3):
  ...
  qml.QubitUnitary(cm0,wires=[0,3,4])
  qml.QubitUnitary(cm1,wires=[1,5,6])
  qml.QubitUnitary(cm2,wires=[2,7,8])
