  ...
  for i in range (Max):
    ...
    for j in range (Max):
      ...
      for k in range (Max):
        ...
        coord.append(y)
        coord.append(z)
  ncoord=torch.Tensor(coord).reshape(32768,3).to("cpu")
  return ncoord
...
def Read_Coor(pdb,aty):
  ...
  atm_c=torch.Tensor(coord).reshape(-1,3).to("cpu")
  fr.close()
  return atm_c,torch.Tensor(atm_r).to("cpu")
...
def Main():
  ...
  for i in range (len(pdb)):
    Img=torch.zeros((8,32768)).to("cpu")
    for j in range (8):
